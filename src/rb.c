...
/* Added on 2018/05/29: Find the first object that is greater than key. */
void *rb_isam_find (const struct rb_table *tree, const void *key)
{
	const struct rb_node *p, *q;
	int cmp, dir;
	void *next;

	assert (tree != NULL && key != NULL);
	for (p = tree->rb_root; p != NULL; ){
		cmp = tree->rb_compare(key, p->rb_data, tree->rb_param);
		if(cmp == 0){
			return p->rb_data;
