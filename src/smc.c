...
static inline void free_node(struct config *cfg, struct node *nd)
{
#ifdef DEBUG
...
static inline struct node *alloc_node(struct config *cfg, int type, int pop, double t)
{
	struct node *nd;
...
struct coal_node *__absorption(struct genealogy *G, struct node *e1, struct node *n2, int pop, double t)
{
	...
	nd = (struct coal_node *)alloc_node(G->cfg, NODE_COAL, pop, t);
...
#endif
	nd = (struct migr_node *)alloc_node(G->cfg, NODE_MIGR, spop, t);
...
//ndiscard_merge++;
	...
	for(i = 0; i < npop_all; i++){
		if(F[i].n > 0){
			...
			ndum = alloc_node(cfg, NODE_DUMMY, nd->pop, nd->t);
			ndum->in = NULL;
...
int simulate(struct genealogy *G, struct profile *prof)
{
	...
	cfg = G->cfg;
	for(i = 0; i < nfrag; i++){
		nds[i] = (struct sam_node *)alloc_node(cfg, NODE_SAM, fgi[i].pop, 0);
		nds[i]->fgid = i;
	}
