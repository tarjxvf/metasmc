...
void dump_edges(struct genealogy *G)
{
	...
	while(l){
		e = (struct edge *)GET_OBJ(l);
		fprintf(stderr, "->%x(%.10f, %.10f)", e, e->bot->t, e->top->t);
		l = l->next;
	}
	fprintf(stderr, "\n");
...
//				fprintf(stderr, "%x(%.10f, %.10f, %d, %d),", e, e->bot->t, e->top->t, e->bdeleted, e->inqueue);
				fprintf(stderr, "%x(%.10f, %.10f, %d),", e, e->bot->t, e->top->t, e->idx);
...
void remove_coal_node(struct genealogy *G, struct coal_node *nd, int iout)
{
	struct node *n2;
	struct edge *e;
...
/* Another version of erase_dangling that try to retain coalescent entanglement. */
void erase_dangling_e(struct genealogy *G, struct edge *e)
{
	struct config *cfg;
	struct migr_node *nm;
...
//		free_edge(G, e);
		e = nm->in;
		if(((struct event *)nm->ev)->type == EVENT_MIGR)
			remove_event(G, (struct event *)nm->ev);
		free_node(G, (struct node *)nm);
	}

	/* If the erased lineage is below root, then move root. */
	if(e->top != G->root){
		remove_coal_node(G, (struct coal_node *)e->top, e->itop);
	}
...
//		free_edge(G, e);
	...
	if(e->top == G->root){
		...
		do{ /* Move downward along another branch to find new root. */
			nbot = ebelow->bot;
...
/* Erase dangling lineage until a coalescent node is reached. */
void erase_dangling(struct genealogy *G, struct edge *e)
{
	struct config *cfg;
	struct migr_node *nm;
...
//		free_edge(G, e);
	...
	if(e->top == G->root){
		...
		pop = e->bot->pop;
		nc = (struct coal_node *)G->root;
		e2 = nc->out[1 - e->itop];
		n2 = e2->bot;
//		e2->bdeleted=1;
		remove_event(G, (struct event *)nc->ev);
		free_node(G, (struct node *)nc);
		while(n2->type == NODE_MIGR){ /* Move downward along another branch to find new root. */
			nm = (struct migr_node *)n2;
			n2 = nm->out->bot;
//			e2->bdeleted = 1;
			if(((struct event *)nm->ev)->type == EVENT_MIGR)
				remove_event(G, (struct event *)nm->ev);
...
void build_trunk(struct genealogy *G, int lb)
{
	struct config *cfg;
	struct list_head *nl, *next;
	int pop;
	...
	while(nl){
		...
		nd = (struct sam_node *)GET_OBJ(nl);
#ifdef DEBUG
		fprintf(stderr, "%d: node=%x, frag end=%d\n", __LINE__, nd, nd->fg->end);
#endif
		if(nd->fg->end <= lb){
			struct edge *e;
#ifdef DEBUG
...
//fprintf(stderr, "%d: g=%.10f, G->total=%.10f", __LINE__, g, G->total);
	...
	while((ev->t - t) * n <= g){
#ifdef DEBUG
		fprintf(stderr, "%d: n=%d, g=%.6f, t=%.6f", __LINE__, n, g, t);
	...
	/* Compute real time and edge index. *
	c = (int)(g / (ev->t - t));
#ifdef DEBUG
	fprintf(stderr, "%d: (%d, %.6f)\n", __LINE__, ev->type, ev->t);
	fprintf(stderr, "%d: n=%d, g=%.6f, t=%.6f, c=%d\n", __LINE__, n, g, t, c);
	for(i = 0; i < cfg->npop + cfg->nsplt; i++)
		fprintf(stderr, ",G->pops[%d].n=%d", i, G->pops[i].n);
	fprintf(stderr, "\n");
#endif
	...
	i = 0;
	for(i = 0; i < cfg->npop + cfg->nsplt; i++){
		e = pick_ith_tedge(G->e_list.front, c, t);
		c -= G->pops[i].n;
		if(e)
			break;
	}
//fprintf(stderr, "%.10f\t%d\t%.10f\t%.10f\n", t, e->id, e->bot->t, e->top->t);
...
//			mg = -log(dunif01()) / dmig;
		...
		fprintf(stderr, "%d:pop=%d, t=%.6f, at=%.6f, mg=%.6f\n", __LINE__, pop, t, at, mg);
#endif
...
//					// e_old->top is the new coalescent node which has to be removed.
					remove_coal_node(G, AS_COAL_NODE(e_old->top), e_old->itop);
