...
struct genealogy *alloc_genealogy(struct config *cfg, struct profile *prof)
{
	...
	G->cfg = cfg;
	G->node_cache[NODE_COAL] = cache_create(sizeof(struct coal_node), cfg->maxfrag * 4);
	G->node_cache[NODE_MIGR] = cache_create(sizeof(struct migr_node), cfg->maxfrag * 2);
//	G->node_cache[NODE_XOVER] = cache_create(sizeof(struct xover_node), cfg->maxfrag * 4);
	...
	G->node_cache[NODE_FLOAT] = G->node_cache[NODE_MIGR];
