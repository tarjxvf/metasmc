...
void rbindex_seq_on(struct rbindex *eidx)
{
	rbindex_setflag(eidx, RBINDEX_SEQUENTIAL);
}

/* Turn off sequential mode. */
void rbindex_seq_off(struct rbindex *eidx)
{
	rbindex_clearflag(eidx, RBINDEX_SEQUENTIAL);
	rbindex_rebuild_tree(eidx);
}

int rbindex_s_first(struct rbindex *eidx, rb_traverser cur)
{
	return cur == &eidx->ls.front;
}
...
void eindex_s_seek(struct rbindex *eidx, double t)
{
	if(rbindex_isseq(eidx)){
		...
		lbwd = eidx->cur_s;
		ebwd = rbindex_cur(lbwd);
		while(ebwd->top->t >= t) ebwd = rbindex_prev(&lbwd);
		lfwd = lbwd;
		efwd = rbindex_cur(lfwd);
		while(efwd->top->t < t) efwd = rbindex_next(&lfwd);
		eidx->cur_s = lfwd;
	}
}
...
static inline void eindex_insert(struct rbindex *eidx, struct edge *e)
{
	if(rbindex_isseq(eidx)){
		...
		cur = eidx->cur_s;
		efwd = rbindex_cur(cur);
		while(efwd->top->t == e->top->t && efwd->bot->t < e->bot->t) efwd = rbindex_next(&cur);
		while(efwd->top->t == e->top->t && efwd->bot->t == e->bot->t && efwd->eid < e->eid) efwd = rbindex_next(&cur);
...
static inline struct rbindex *eindex_create(struct genealogy *G, int pop)
{
	struct rbindex *eidx;
	struct node *top, *bot;
	struct edge *e;
...
void erase_dangling2(struct genealogy *G, struct edge *e)
{
	...
	while(etmp->top->type == NODE_MIGR){
		rbindex_delete(G->pops[etmp->bot->pop].eidx, etmp);
		etmp = etmp->top->in;
	}
	rbindex_delete(G->pops[etmp->bot->pop].eidx, etmp);
...
#endif
	eabove = ntop->in;
	rbindex_delete(G->pops[eabove->bot->pop].eidx, eabove);
	ebelow = ntop->out[1 - etmp->itop];
	rbindex_delete(G->pops[ebelow->bot->pop].eidx, ebelow);
...
//	if(n > nthres){
	if(avg1 < avg2){
		...
		if(rbindex_isseq(pop->eidx)){
			...
			key.top = &top;
			key.bot = &bot;
			e = rbindex_find(&tr, pop->eidx, &key);
#ifdef DEBUG
...
#endif
//	rbindex_delete(G->pops[e->bot->pop].eidx, e);
...
//	eindex_insert(G->pops[f->bot->pop].eidx, f);
//	eindex_insert(G->pops[nd->out[0]->bot->pop].eidx, nd->out[0]);
...
//	}
	...
	rbindex_s_set(G->pops[e->bot->pop].eidx, e);
...
//	cur = G->pops[e->bot->pop].eidx->cur_s;
//	efwd = rbindex_cur(cur);
//	while(efwd->top->t >= e->top->t && efwd->bot->t < e->bot->t && efwd->eid < e->eid) efwd = rbindex_next(&cur);
//	rbindex_insbefore(cur, e);
...
struct coal_node *coalescent( struct genealogy *G, struct edge_set *F, int pop, double t)
{
	...
	// Move cursor of sequential mode.
//	for(i = 0; i < G->cfg->npop_all; i++)
//		eindex_s_move(G->pops[i].eidx, t);
...
void reset_populations(struct genealogy *G)
{
	...
	for(pop = 0; pop < cfg->npop; pop++){
		...
		rbindex_s_reset(G->pops[pop].eidx);
	}
...
//				}
				...
				rbindex_delete(G->pops[nxover->pop].eidx, nxover->in);

				rbindex_delete(G->pops[e2->bot->pop].eidx, e2);
				...
				rbindex_delete(G->pops[nxover->pop].eidx, nxover->in_new);

				if(e2 == e){	// Floating lineage is absorbed to the same lineage (loop in ARG)
					struct edge *e_old;

					rbindex_delete(G->pops[pop].eidx, e);
					rbindex_delete(G->pops[pop].eidx, AS_COAL_NODE(e2->bot)->out[0]);
					rbindex_delete(G->pops[pop].eidx, AS_COAL_NODE(e2->bot)->out[1]);
...
//					// e_old->top is the new coalescent node which has to be removed. In this case, e2 must be in local genealogy, so tsindex must be updated
...

				{
					...
					e_new = nxover->in_new;
					rbindex_delete(G->pops[e_new->bot->pop].eidx, e_new);
					e_below = nxover->out;
...
//					list_add(&G->e_list, edum);
				...
				rbindex_delete(G->pops[edum->bot->pop].eidx, edum);
				...
				if(ev->type == EVENT_DXVR){
					...
					erm = ndum->in;
					while(erm){
						rbindex_delete(G->pops[erm->bot->pop].eidx, erm);
						erm = erm->top->in;
					}
...
//			return -1;
		...
		if(t + minu < ev->t || t + minv < ev->t || t + minz < ev->t){
			...
			if(minu < minv && minu < minz){	// Absorption
				...
				rbindex_s_set(G->pops[upop].eidx, e);
				...
				evnew = (struct event *)nd->ev;
				rbindex_s_set(G->pops[vpop].eidx, nd->out[0]);
...
//				fprintf(stdout, "nmigr=%d, dpop=%d, c=%d, ev->dpop=%d, ev->spop=%d\n", nmigr, dpop, c, ev->dpop, ev->spop);
				...
				rbindex_s_set(G->pops[zpop].eidx, nd->out);
